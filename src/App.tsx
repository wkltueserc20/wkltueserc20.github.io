...
function App() {
  ...
  const handleEditVaccine = (record: Record, newEndTimestamp: number, newNote: string, newSubType: string, newLabel: string, newAmount?: number) => {
    const updatedRec = { ...record, endTimestamp: newEndTimestamp, note: newNote, subType: newSubType, label: newLabel, amount: newAmount, updatedAt: Date.now() };
    updateRecord(updatedRec);
    haptic();
    const updated = records.map(r => r.id === record.id ? updatedRec : r);
    fullSync(updated, setAllRecords);
