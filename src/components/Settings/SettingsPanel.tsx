      ...
      <div className={cardCls}>
        ...
        <div className="flex items-center justify-between px-1">
          ...
          <div className="flex items-center gap-2 text-xs text-slate-500">
            ...
            <button
              onClick={() => setBabyInfo({ ...babyInfo, quietHourDisabled: !babyInfo.quietHourDisabled })}
              className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${
                babyInfo.quietHourDisabled ? 'bg-slate-300 dark:bg-slate-600' : 'bg-indigo-500'
              }`}
            >
              <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white shadow transition-transform ${
                babyInfo.quietHourDisabled ? 'translate-x-1' : 'translate-x-[18px]'
              }`} />
            </button>
          </div>
        </div>
